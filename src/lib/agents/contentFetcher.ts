...
export async function fetchAndParseContent(result: SearchResult): Promise<Document | null> {
  try {
    ...
    console.error(`DOCUMENT_FAILED: url=${result.url}, message=${error.message}`);
    return null;
  }
}